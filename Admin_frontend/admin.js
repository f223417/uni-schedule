...
<<<<<<< HEAD
  ...
  if (isNaN(week1StartDate.getTime())) {
    console.error('Week 1 start date not set or invalid');
    weekValueDisplay.textContent = `Week ${weekNumber}`;
    return;
  }
  
  // Calculate the start date for the current week
  const currentWeekStartDate = new Date(week1StartDate);
  currentWeekStartDate.setDate(week1StartDate.getDate() + (weekNumber - 1) * 7);
...
function setWeekStartDate() {
  ...
  // Save the start date in localStorage
  localStorage.setItem('week1StartDate', startDate.toISOString());
...
<<<<<<< HEAD
  ...
  if (pdfBtn) {
    if (pdfBtn.getAttribute('data-processing') === 'true') {
      console.log('PDF generation already in progress');
      return;
    }
    pdfBtn.setAttribute('data-processing', 'true');
...
<<<<<<< HEAD
  ...
  .finally(() => {
    ...
    if (pdfBtn) {
      pdfBtn.removeAttribute('data-processing');
...
<<<<<<< HEAD
      ...
      function normalizeWeekName(weekStr) {
        ...
        // Extract just "Week X" from strings like "Week 1" or "Week 1 (1/1/1970)"
      ...
      // Group entries by week (normalized)
      const weekGroups = {};
      data.forEach(entry => {
        const normalizedWeek = normalizeWeekName(entry.week);
        if (!weekGroups[normalizedWeek]) {
